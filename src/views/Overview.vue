<template>
  <div class="wrapper">
      ...
      <template v-if="superAdmin">
        <b-row v-show="skele[1]" v-for="item,index in allbrand" :key="index" class="mt-2">
          <b-col cols="12"><h5 class="text-bold">{{item.brand}}</h5></b-col>
          <template v-for="city,i in item.locations">
            <b-col cols="12" lg="6" class="mt-3 imgCard" :key="i" @click="viewDash(city)">
                  ...
                  <b-row>
                    ...
                    <b-col cols="8">
                      <div class="float-right text-light">
                        <font-awesome-icon fixed-width icon="power-off"/> {{$t('nav.'+item.mStatus[i])}} /
                        <font-awesome-icon fixed-width icon="link"/> {{$t('nav.'+item.rStatus[i])}}
                      </div>
                    </b-col>
                  </b-row>
                </b-card-footer>
                  ...
                  <b-row>
                    ...
                    <b-col cols="8">
                      <div class="float-right text-light">
                        ...
                        <font-awesome-icon fixed-width icon="power-off"/> {{$t('nav.'+mStatus[index])}} /
                        <font-awesome-icon fixed-width icon="link"/> {{$t('nav.'+rStatus[index])}}
                      </div>
                    </b-col>
                  </b-row>
                </b-card-footer>
...
<script>
  ...
  import Vue from 'vue'
	...
	export default {
        ...
        cityImg: [],
        cups: [0,0,0,0],
        mStatus: ['offline','offline','offline','offline'],
        rStatus: ['not synced','not synced','not synced','not synced'],
        superAdmin: false,
        brands: []
			}
		},
            ...
            await db.doc('status/'+name[x].brand+'-'+name[x].locations[y]).get().then((doc) => {
              if(doc.exists){
                ...
                if(doc.data()['Recipe-Update']){
                  name[x].rStatus.push('synced');// this.rClass = 's-on';
                }else{
                  name[x].rStatus.push('not synced');// this.rClass[x] = 's-off';
                }
              }else{
      ...
      async getBanner(){
        ...
        for(x in city){
          ...
          await axios.get(URL).then(response => {
            ...
            console.log(error);
          });//.finally(() => this.setLoadingState(false,true));
        }
      },
      async getMStatus(){
        var x = 0,
